...
export interface Job {
    mainStat?:Stat | Stat[];
    subStat?:Stat;
    apStatPerLevel:(level:number)=>Stats;
    atkType:"공격력"|"마력";
    ...
    defaultSkills?:string[];//캐릭터가 기본으로 갖는 스킬. 4차스킬과 하이퍼스킬
...
export const jobDict:{[index:string]:Job} = {
    "히어로":{
        mainStat:"STR",
        subStat:"DEX",
        apStatPerLevel:(level)=>({"APSTR":18+(level*5)}),
        atkType:"공격력",
    ...
    "아크메이지(불,독)":{
        mainStat:"INT",
        subStat:"LUK",
        apStatPerLevel:(level)=>({"APINT":18+(level*5)}),
        atkType:"마력",
    ...
    "나이트로드":{
        mainStat:"LUK",
        subStat:"DEX",
        apStatPerLevel:(level)=>({"APLUK":18+(level*5)}),
        atkType:"공격력",
        unionStat:[{"크리티컬 확률":1}, {"크리티컬 확률":2}, {"크리티컬 확률":3}, {"크리티컬 확률":4}, {"크리티컬 확률":5}]
    },
    "섀도어":{
    ...
    "데몬어벤져":{
        ...
        subStat:"STR",
        atkType:"공격력",
        apStatPerLevel:(level)=>({"APHP":1000+(level*75)}),
        calcDmg:(statInfo,classes)=>{
...
export const defaultCalcBonusStatGradeFomula = (statInfo:Stats, classes:Job) => {
    return (statInfo[classes.mainStat as Stat] ?? 0) +
