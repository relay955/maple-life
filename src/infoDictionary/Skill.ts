...
export interface Skill{
  ...
  enhanceSkill?:string;//코강으로 강화 가능한 스킬인경우 해당스킬 명시
  autoActiveSkill?:{[index:string]:number};//스킬 사용시 자동으로 발동되는 스킬 목록. 스킬명:확률. 0~1사이로 표기

  buffStat?:Stats//스킬 사용시 버프형태로 일정시간동안 제공되는 스텟
  buffDuration?:number;//버프 지속시간. 지속시간동안 버프 적용
  isMatrixSkillCoreNeeded?:boolean;//코어 없으면 아예 사용불가한 스킬인경우 true
}
